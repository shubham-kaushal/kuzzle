...
const { kebabCase, camelize } = require('../../util/inflector');
...
class PluginsManager {
  ...
  async _initApi (plugin) {
    for (const [controller, definition] of Object.entries(plugin.instance.api)) {
      ...
      this.registerEmbeddedSDKController(plugin.name, controller, definition);
  ...
  registerEmbeddedSDKController (pluginName, controller, definition) {
    ...
    const className = 'EngineController';
    const name = 'engine';
    const args = [
    ...
    const args2 = ['index', 'collection', 'body']
    const options = [
      '_id',
      'refresh'
    ]
    const Controller = new Function('BaseController', `
      return class ${className} extends BaseController {
        constructor (kuzzle) {
          super(kuzzle, '${name}');
        }
      }
    ...
    Controller.prototype.create = new Function(...args2, 'options = {}', `
      return this.query({
        action: 'create',
        ${args2.join(', ')},
        ${options ? options.map(o => `${o}: options.${o}`).join(',') : ''}
      })
    `)


    const engine = new Controller('kuzzle');

    console.log(engine.create('iot', 'sensors', {lol:42}, {_id: 'lol'}))
  }
