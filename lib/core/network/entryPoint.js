...
class EntryPoint {
   ...
   * @param {object} extra
   */
  logAccess (connection, request, responseSize, extra = null) {
    ...
    // user init: use the already decoded and verified token stored in
    // the request
    // If not available, then that may mean that Kuzzle didn't verify the user
    let user = null;
    ...
    let offset = Math.max(connection.ips.length - 1 - this.config.logs.accessLogIpOffset, 0);
    const ip = connection.ips[offset] || '-';
    ...
    this.logger.info(`${ip} - ${user} [${ts}] "${verb} ${url} ${protocol}" ${status} ${responseSize} ${referer} ${agent}`);
  }
