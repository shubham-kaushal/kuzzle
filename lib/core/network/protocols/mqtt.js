...
class MqttProtocol extends Protocol {
  ...
  onMessage (packet, client) {
    ...
    try {
      ...
      this.entryPoint.execute(request, response => {
        this._respond(connection, client, request, response);
      });
    }
    catch (error) {
      this._respondError(connection, client, error);
    }
  }

  _respond (connection, client, request, response) {
    ...
    const payload = Buffer.from(JSON.stringify(response.content));

    this.entryPoint.logAccess(connection, request, payload.byteLength);
  ...
  _respondError (connection, client, error) {
    ...
    this._respond(
      ...
      client,
      errReq,
      removeErrorStack(errReq.response.toJSON()));
  }
