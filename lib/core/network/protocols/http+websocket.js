...
class HTTPWS extends Protocol {
  ...
  wsOnMessageHandler (socket, data) {
    ...
    if (this.wsConfig.rateLimit > 0) {
      if (socket.last === this.entryPoint.now) {
        socket.count++;
        ...
        socket.last = this.entryPoint.now;
        socket.count = 1;
      }
    }
    ...
    // this.wsSend(socket, Buffer.from('{"status":200,"result":{},"error":null}'));
    // return;
    this.entryPoint.execute(request, result => {
      ...
      const payload = Buffer.from(JSON.stringify(result.content));
      this.entryPoint.logAccess(connection, request, payload.byteLength);

      this.wsSend(socket, payload);
    });
  }
